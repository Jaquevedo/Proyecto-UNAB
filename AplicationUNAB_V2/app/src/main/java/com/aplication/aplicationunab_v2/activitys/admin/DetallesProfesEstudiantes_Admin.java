...
public class DetallesProfesEstudiantes_Admin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onClick(View view) {
                db.collection("Docentes").document(UIDoc.getText().toString()).update("nombre",nameDoc.getText().toString(),"pass",passDoc.getText().toString(),"doc",docDoc.getText().toString(),"email",emailDoc.getText().toString(),"programa",progDoc.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        ...
                        Intent it1 = new Intent(DetallesProfesEstudiantes_Admin.this, ListaProfesoresAdmin.class);
                        startActivity(it1);
                    }
        ...
        btnBorr.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                AlertDialog.Builder builder = new AlertDialog.Builder(DetallesProfesEstudiantes_Admin.this);
                builder.setMessage("¿Desea eliminarlo de la base de datos?").setPositiveButton("Eliminar", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        db.collection("Docentes").document(UIDoc.getText().toString()).delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                Toast.makeText(context, "Eliminado de la base de datos", Toast.LENGTH_SHORT).show();
                                Intent it = new Intent(context, ListaProfesoresAdmin.class);
                                startActivity(it);
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Log.w("establecimientosAct","Error "+ e);
                            }
                        });
                    }
    ...
    public void viewOption(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.BackListDoc:
                Intent intentback = new Intent(DetallesProfesEstudiantes_Admin.this, ListaProfesoresAdmin.class);
